...
const useRemainingDaysInMonth = (month: number) => {
  ...
  const [remainingDays, setRemainingDays] = useState<{ day: string; num: number }[]>([]);
  ...
  useEffect(() => {
    const calculateRemainingDays = () => {
      ...
      if (month === 0) {
        ...
        for (let dayNumber = currentDate.getDate(); dayNumber <= lastDayOfMonth; dayNumber++) {
          const currentDay = new Date(year, currentMonth - 1, dayNumber);
          const dayString = currentDay.toLocaleDateString('es-ES', { weekday: 'long' });

          days.push({
            day: dayString?.split(',')?.shift()?.trim()?.slice(0, 3) + '.' ?? '',
            num: dayNumber,
          });
        }
